    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let already_running = match get_api_health(&port) {
                ...
                Err(err) => {
                    // Check if the TCP connection failed (port closed) vs
                    // some other service answered (port occupied but not our API).
                    let is_port_in_use = err.contains("Connection refused") == false
                        && !err.contains("10061")
                        && !err.contains("actively refused");
                    if is_port_in_use && !err.contains("no JSON body") {
                        let msg = format!(
                            "MyNotes AI 后端启动失败：8000 端口已被其他程序占用。\
                             请关闭占用 8000 端口的程序后重试。\n\n诊断：{err}"
                        );
                        write_log(&log_path, format!("preflight health check: PORT CONFLICT — {err}"));
                        show_error("MyNotes AI", &msg);
                        return Err(std::io::Error::new(std::io::ErrorKind::AddrInUse, msg).into());
                    }
                    write_log(
                        &log_path,
