  ...
  async function saveModelSettings() {
    try {
      ...
      if (err instanceof ApiNetworkError) {
        ...
        const detailMsg = detail?.detail ?? detail?.message ?? '';
        if (err.status === 422) {
          setSettingsStatus(`设置字段格式错误: ${JSON.stringify(detailMsg || detail)}`);
        } else if (err.status === 500) {
          setSettingsStatus(`后端保存设置失败: ${detailMsg || err.status}`);
        } else {
          setSettingsStatus(`保存设置失败 (${err.status}): ${detailMsg || ''}`);
        }
      } else {
